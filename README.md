...
### 五笔输入
- **五笔 86 方案**，端侧分片约 **9.6 万条目**（单字 + 全部两 / 三字词 + 高频补丁），云端按需提供四字以上长词、成语、地名、品牌与长尾。
- 一级 / 二级 / 三级简码与全码完整；**四码时词组优先**，简码时高频锚字置顶。
- **词频自学习**：刚上屏的字 / 词再次输入自动靠前，最近上屏优先。
- 严格遵循使用规则：有编码时回车只上屏编码、空格上首候选、点选上屏、空候选回车换行；**词库仅含简体字**。

### 五笔 + 拼音混打
- **默认混模式**：五笔、拼音、英文、数字无需切换即可混合输入；普通框始终默认混模式，无需记忆当前在哪一档，手动临时切换不跨输入框。
- 拼音支持**全拼、双拼（小鹤 / 自然码 / 微软等多套方案）、简拼**。
- **拼音容错**：精确无结果时自动做音节级纠错，漏字母 / 口音 / 手滑（如 `bejng`→「北京」）也能出字。
- 英文支持自动大小写、单词与短句补全；可对文本框词组做**翻译**，跟随光标实时变化。
...
### 语境联想与本地学习
- **语境整句联想**：通读整句 / 上下文给出接下来可能要说的整句（「喉咙很疼」→「有可能感冒了，我得去看医生」），而非单字组词。
- **本地高频词学习**：从文本框真实上屏文本划词，7 天窗、高频阈值、上限 500 条，越用越快，全程本地、默认不上传。
- **一键诊断报告**：设置里生成异常 / 性能 / 环境 / 脱敏操作轨迹报告，一键复制或分享，便于反馈定位。

### 安全与触感
- **密码框适配**：文本密码 / 数字密码下不自动大写首字母、不读取剪贴板、不做语义泄露。
- 按键触点视觉反馈 + **轻微震动**（可在设置中关闭）。
- 多套玻璃主题：浅色清透玻璃（v2.2 起默认，v2.3 升级为图一透光质感）、深色鲜艳果冻、深空冰蓝、幻紫全息。
